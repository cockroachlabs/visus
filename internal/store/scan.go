...
func (s *store) DeleteScan(ctx context.Context, name string) error {
	...
	if err != nil {
		log.Errorf("delete patterns error:%s", err)
		txn.Rollback(ctx)
		return err
	}
	_, err = txn.Exec(ctx, deleteScanStmt, name)
	if err != nil {
		log.Errorf("delete scan error:%s", err)
		txn.Rollback(ctx)
		return err
	}
	return nil
...
func (s *store) GetScan(ctx context.Context, name string) (*Scan, error) {
	scan := &Scan{}
	log.Trace(getScanStmt)
	rows, err := s.conn.Query(ctx, getScanStmt, name)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	if rows.Next() {
		err := rows.Scan(
			&scan.Name, &scan.Path, &scan.Format, &scan.LastModified, &scan.Enabled)
		if err != nil {
			return nil, err
		}
