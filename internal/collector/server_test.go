...
func TestClusterScopeCollectors(t *testing.T) {
	...
	// When leader: both collections should be scheduled.
	srv.isLeader.Store(true)
	...
	a.True(ok, "cluster-scoped collection should be scheduled on leader")
	a.Equal(2, len(srv.scheduler.Jobs()))

	// When not leader: only node-scoped should remain.
	srv.isLeader.Store(false)
	r.NoError(srv.Refresh(stop))
	_, ok = srv.getJob(nodeColl.Name)
	a.True(ok, "node-scoped collection should still be scheduled")
	_, ok = srv.getJob(clusterColl.Name)
	a.False(ok, "cluster-scoped collection should be removed on non-leader")
...
func TestLeaderTransition(t *testing.T) {
	...
	// Start as non-leader: cluster-scoped should not be scheduled.
	srv.isLeader.Store(false)
	r.NoError(srv.Refresh(stop))
	_, ok := srv.getJob(clusterColl.Name)
	a.False(ok, "cluster-scoped should not be scheduled on non-leader")
	a.Equal(0, len(srv.scheduler.Jobs()))

	// Transition to leader: cluster-scoped should be added.
	srv.isLeader.Store(true)
	r.NoError(srv.Refresh(stop))
	_, ok = srv.getJob(clusterColl.Name)
	a.True(ok, "cluster-scoped should be scheduled after becoming leader")
	a.Equal(1, len(srv.scheduler.Jobs()))

	// Transition back to non-leader: cluster-scoped should be removed.
	srv.isLeader.Store(false)
	r.NoError(srv.Refresh(stop))
	_, ok = srv.getJob(clusterColl.Name)
	a.False(ok, "cluster-scoped should be removed after losing leader status")
	a.Equal(0, len(srv.scheduler.Jobs()))
}
